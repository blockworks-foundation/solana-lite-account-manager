...
pub async fn main() {
    ...
    assert!(PathBuf::from_str(&snapshot_dir).unwrap().is_dir(), "snapshot_dir must exist and must be a directory");
    assert!(PathBuf::from_str(&accounts_path).unwrap().is_dir(), "accounts_path must exist and must be a directory");
    ...
    info!("Loading from snapshot archive {}, unpacking to {} with accounts in {}",
        snapshot_archive_path_file.display(), snapshot_dir, accounts_path.display());
    ...
    info!("unarchived_full_snapshot: {:?}", unarchived_full_snapshot.storage.len());
